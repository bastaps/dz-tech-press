...
export function exportJSON() {
  downloadBlob(new Blob([JSON.stringify(DATASET, null, 2)], { type: 'application/json' }), 'observatoire_du_marche_de_la_telephonie_mobile_mpdupq1v.json');
  toast('JSON exporté');
}

export function exportCSV() {
  const rows = DATASET.indicateurs.map(d => ({ Indicateur: d.label, Valeur: d.valeur, Unité: d.unite }));
  downloadBlob(new Blob([toCSV(rows)], { type: 'text/csv;charset=utf-8' }), 'observatoire_du_marche_de_la_telephonie_mobile_mpdupq1v_indicateurs.csv');
  toast('CSV exporté');
}
...
export async function exportPDF() {
  ...
  try {
    ...
    pdf.save('observatoire_du_marche_de_la_telephonie_mobile_mpdupq1v.pdf');
    toast('PDF exporté');
