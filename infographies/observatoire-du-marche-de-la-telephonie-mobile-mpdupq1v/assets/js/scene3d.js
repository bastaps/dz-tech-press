/**
 * Scène 3D — « Constellation de données Téléphonie Mobile »
 * Three.js 0.160 — Hover interactif avec étiquettes KPI fluides
 * Algeria Tech Generator
 */
...
export function initScene3D(container) {
  ...
  crystal.userData.kpi = {
    label: 'Marché Mobile Algérie',
    valeur: '55,94 M',
    unite: 'abonnés · T3 2025'
  };
  scene.add(crystal);
  ...
  // Anneau lumineux autour du cristal
  const ringGeo = new THREE.TorusGeometry(3.2, 0.04, 8, 80);
  const ring = new THREE.Mesh(ringGeo, new THREE.MeshBasicMaterial({
    color: c1, transparent: true, opacity: 0.35
  }));
  ring.rotation.x = Math.PI / 2;
  scene.add(ring);

  // ── Sphères orbitales KPI ───────────────────────────────────────────
  ...
  const orbs = kpis.map((kpi, i) => {
    ...
    // Halo lumineux (double sphère back-face)
    ...
    // Orbite décorative (cercle de référence)
    const orbitPts = [];
    const steps = 128;
    ...
    // Ligne de connexion cristal ↔ sphère
    ...
    const conn = new THREE.Line(connGeo, new THREE.LineBasicMaterial({
      color: orbColors[i], transparent: true, opacity: 0.45
    }));
    scene.add(mesh);
    scene.add(conn);
    return { mesh, conn, radius, phi: (i / 3) * Math.PI * 2, spd: 0.26 + i * 0.09, kpi };
  });

  // ── Surface sinusoïdale (grille de fond) ────────────────────────────
  ...
  scene.add(new THREE.Mesh(fGeo, new THREE.MeshStandardMaterial({
    color: c2, wireframe: true, transparent: true, opacity: 0.09
  })));
  ...
  // ── Tooltip hover — étiquette KPI fluide ────────────────────────────
  ...
  tt.style.cssText = [
    ...
    'box-shadow:0 6px 32px rgba(0,0,0,.65),0 0 0 1px rgba(212,164,55,.12)',
